...
public class EmployeeSearchDto extends GenericSearchDto<Employee>{
    ...
    protected void addFilters(Root<Employee> root,
                              CriteriaQuery<?> query,
        ...
        if (firstName != null) {
            ...
            filters.add(criteriaBuilder.equal((criteriaBuilder.lower(root.get("firstName"))),firstName.toLowerCase())
            );
        }

        if (lastName != null) {
            log.debug("Filtering on Employee last name: {}", lastName);
            filters.add(criteriaBuilder.equal((criteriaBuilder.lower(root.get("lastName"))),lastName.toLowerCase())
            );
        }
